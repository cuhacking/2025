...
import { getDatePostfix, getDayOfMonth, getMonth } from '@cuhacking/shared/utils/date.helpers'
...
export function EventItem({ event, className }: EventProps) {
  ...
  const buttonMessage = event.status === 'upcoming' ? 'REGISTER NOW' : 'VIEW PHOTOS'
  return (
        ...
        {event.status !== 'in-progress'
          ? (
              <div className="flex justify-end w-full pt-1 text-xl">
                <Button variant="default" size="sm"><Link to={event.link} target="_blank">{buttonMessage}</Link></Button>
              </div>
            )
          : null}
      </article>
