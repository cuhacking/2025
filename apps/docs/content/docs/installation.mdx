...
<Tabs groupId="projects" items={['Graph', 'Storybook', 'Docs', 'Portal', 'Website','Slides', 'CMS (Content Management System)', 'ESLint Config Inspector']} persist>
...
```
  ...
  <Tab value="CMS (Content Management System)">
    <Tabs groupId="cms-project-nx-commands" items={['dev', 'build', 'start']} persist>
      <Tab value="dev">
        ```sh title="Nix Shell"
        pnpm nx dev cms --verbose
        ```
      </Tab>
      <Tab value="build">
        ```sh title="Nix Shell"
        pnpm nx build cms --verbose
        ```
      </Tab>
      <Tab value="start">
        ```sh title="Nix Shell"
        pnpm nx start cms --verbose
        ```
      </Tab>
      </Tabs>
