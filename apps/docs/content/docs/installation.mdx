...
<Tabs groupId="projects" items={['Graph', 'Arch', 'Storybook', 'Docs', 'Portal', 'Website','Slides', 'API', 'ESLint Config Inspector']} persist>
...
```
  ...
  <Tab value="API">
    <Tabs groupId="api-project-nx-commands" items={['dev', 'build', 'start']} persist>
      <Tab value="dev">
        ```sh title="Nix Shell"
        pnpm nx dev api --verbose
        ```
      </Tab>
      <Tab value="build">
        ```sh title="Nix Shell"
