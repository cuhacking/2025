...
export async function globalCooldownFN(client: DiscordClient, message: Message | Interaction<'cached'>, command: AnyCommand, interactionType: InteractionTypeOptions): Promise<boolean> {
  if (!command.globalCooldown || Number.isNaN(command.globalCooldown))
    return true
  ...
  if (Math.floor(currentTime - storedTime) >= command.globalCooldown || !storedTime) {
    ...
    if (command.returnErrors === false || command.returnGlobalCooldownError === false)
      return false
